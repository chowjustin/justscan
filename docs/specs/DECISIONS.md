...
| Date | Reversal |
|---|---|
| 2026-08-28 | `00_foundations.md` §10 step 10's `+0 opening` row. Step 1 starts from an empty catalogue with no seed and step 2 creates Chitato with "no movement written", while §9 and R-03-13 both state that zero stock is the *absence* of movements, never a movement of zero. The row could not exist under the system's own rules — three sources disagreed and the two rules win. Step 10 now reads three movements, not four. This closes the open question carried since session 3. |
| 2026-08-28 | `STRUCTURE.md`'s placement of `GoldenPathTests.swift` in session 5. 05 §12 defines no acceptance criterion for it and 05 §13's build checklist does not list it, so it is not part of this module's contract; it belongs to the `/golden-path` gate that runs after 05. Reassigned rather than built early. |
| 2026-08-28 | `05 §10`'s summary card, which the region table listed unconditionally while §3.4 described `Semua` as grouped-with-subtotals only. An all-time total over a `limit: 100` window is wrong, and a true one needs the unbounded fetch foundations §8 forbids. The card is now a *day* summary and appears on `Hari Ini` and `Kemarin` only. |
| 2026-08-28 | `05 §10`'s per-day subtotal under `Semua`, read against §8's paging. A 100-row page almost always ends mid-day, so the oldest group's subtotal would be a partial figure presented as a day total. It is withheld until the day is fully loaded. |
